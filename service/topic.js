...
module.exports = {
  async save(topic) {
    ...
    let tagToCreate = [];
    if (tags.length > 0) {
      const existedTagIds = await db.pluck('id').from('tag').whereIn('id', tags);
      tagIds = tagIds.concat(existedTagIds);
    }
    if (newTags.length > 0) {
      const existedTags = await db.from('tag').whereIn('name', newTags);
    ...
    const topicId = await db.transaction(async (trx) => {
      ...
      if (isEditing) {
        ...
        topicId = await trx.insert(topicData).into('topic');
      }
      ...
      if (tagToLink.length > 0) {
        ...
        await trx.batchInsert(
          'topicTag',
          Array.from(new Set(tagToLink)).map((id) => ({ tagId: id, topicId })),
        );
      }
      return topicId;
    });
  ...
  async list(page, pageSize, whereArgs, orderByArgs, otherArgs) {
    ...
    return {
      list: topics.map((t) => ({
        ...t,
        tags: tags.filter((tag) => tag.topicId === t.id).map(({ id, name }) => ({ id, name })),
  ...
  async view(id) {
    ...
    const user = await db.from('user').where('id', topic.userId);
    return {
      ...topic,
      user: user[0],
      tags,
