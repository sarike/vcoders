  ...
  renderTopic() {
    ...
    return (
      <Fragment>
        ...
        <div className="mb-3">
          <small>
            <a href="#">{user && user.nickName}</a> 发表于 {formatDatetime(createTime)}
          </small>
          <small>
            {stickable && (
              <a href="#" className="ml-3" onClick={(e) => this.handleStick(e, topic)}>
                {topic.isSticked ? '取消置顶' : '置顶'}
              </a>
            )}
            {editable && (
              <a href="#" className="ml-3" onClick={(e) => this.handleEdit(e, topic)}>
                编辑
              </a>
            )}
          </small>
  ...
  render() {
    return (
      <div className="card mb-3 topic-detail">
        <div className="card-body">{this.renderTopic()}</div>
      </div>
    );
  }
}
